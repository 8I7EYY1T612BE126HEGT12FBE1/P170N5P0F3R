...
def license(self, key, hwid=None):
    self.checkinit()
    if hwid is None:
        hwid = win32security.ConvertSidToStringSid(win32security.LookupAccountName(None, os.getlogin())[0])
    ...
    response = self.__do_request(post_data)
    json = jsond.loads(response)
    if json["success"]: self.__load_user_data(json["info"]),print("\n   Valid License Key")
    else:
        if json["message"]=="Invalid license key": print("\n   Invalid License Key"), time.sleep(3), os._exit(1)
